odoo.define('aspl_pos_kitchen_screen_ee.pos',function (require) {
    ...
    models.load_fields("res.users", ['kitchen_screen_user','pos_category_ids','cook_user_ids']);
    /*models.load_fields("pos.order.line", ['state');*/ /*added by Falak/Ahmad 20190718*/
    models.load_fields("pos.order.line", ['state','pos_cid','name','order_line_id']); /*added by Falak/Ahmad 20190718*/   

	chrome.Chrome.include({
		build_widgets:function(){
			var self = this;
			this._super();
			self.call('bus_service', 'updateOption','pos.order.line', 999 ); /*session.uid added by Falak/Ahmad 20190718*/
		...
		_onNotification: function(notifications){
			var self = this;
			console.log('notifications catched');/*added by Falak/Ahmad 20190718*/
			
			console.log('Hellowww');
			console.log(this.pos.user.kitchen_screen_user);
			console.log(this.pos.user.cook_user_ids.length);
			if (this.pos.user.kitchen_screen_user || this.pos.user.cook_user_ids.length != 0) {/*added by Falak/Ahmad 20190718*/
			
			for (var notif of notifications) {
				console.log('step0');/*added by Falak/Ahmad 20190718*/
				console.log(notif);/*added by Falak/Ahmad 20190718*/
				console.log('step0.0');/*added by Falak/Ahmad 20190718*/
				console.log(notif[1].screen_display_data);/*added by Falak/Ahmad 20190718*/
				console.log(this.pos.config.send_to_kitchen);/*added by Falak/Ahmad 20190718*/
				if(notif[1].screen_display_data ){
					console.log('step1',notif[1].screen_display_data.new_order);
					...
					_.each(notif[1].screen_display_data.orders,function(order){
						_.each(order.order_lines,function(line){
						   console.log('line.state',line.state);
					...
					console.log('step57');/*added by Falak/Ahmad 20190718*/
					...
					if(self.pos.get_order_list().length !== 0){
						...
						for (var i = 0; i < collection_orders.length; i++) {
							...
							_.each(collection_order_lines,function(line){
								...
								}*/ /*added by Falak/Ahmad 20190718*/
								if(line.pos_cid === notif[1].order_line_state.pos_cid){
									console.log('step11');
									line.state = notif[1].order_line_state.state;
									/*added by Falak/Ahmad 20190718*/
									line.order_line_id =  notif[1].order_line_state.order_line_id;
	...
	models.PosModel = models.PosModel.extend({
		mirror_kitchen_orders:function(new_order){
		    console.log('mirror_kitchen_orders:function(new_order){');
            rpc.query({
                model: 'pos.order',
    ...
    screens.define_action_button({
        ...
        'condition': function(){
        console.log('pos.user.kitchen_screen_user');
        console.log(this.pos.user.kitchen_screen_user);
...
//    Send order to kitchen
    ...
    models.Orderline = models.Orderline.extend({
        initialize: function(attr,options){
            this.state = 'waiting';
			this.pos_cid = this.get_xx_cid(); /*added by Falak/Ahmad 20190718*/
			this.order_line_id = this.order_line_id
            /*this.name =  Orderline.name;/* added by Falak/Ahmad 20190718*/
        ...
        export_as_JSON: function() {
            ...
            var new_attr = {
                ...
                state : this.state,
                /*pos_cid : this.cid,*//*added by Falak/Ahmad 20190718*/
				pos_cid : this.pos_cid,
    ...
    var kitchenScreenWidget = screens.ScreenWidget.extend({
		...
		show: function() {
			...
			console.log(self.pos.user.kitchen_screen_user);
...
//        	self.pos.mirror_kitchen_orders();
		...
		cancel_order_click:function(event){
			var self = this;
			var order_line_id = parseInt($(event.currentTarget).data('id'));
			console.log('in cancel_order_click:function',order_line_id);
			if(order_line_id){
				rpc.query({
					model: 'pos.order.line',
					method: 'update_orderline_state',
		...
		console.log('cancel_pos_or der: function(event){');
		    var self = this;
		    var cancel_order_id = parseInt($(event.currentTarget).attr('data-pos-order-id'));
  			console.log('in cancel_pos_order:function',cancel_order_id);
            ...
            if(cancel_order_id){
                ...
                rpc.query({
                    ...
                    if(result){
                        ...
                        self.pos.mirror_kitchen_orders(cancel_order_id);
                    }
                }).fail(function(){
                    console.log('result',result);
