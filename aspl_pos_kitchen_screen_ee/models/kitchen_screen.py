...
class PosOrder(models.Model):
    ...
    @api.model
    def cancel_pos_order(self, cancel_order_id):
        print('In Cancel')

        if cancel_order_id:
            cancel_order_obj = self.browse(int(cancel_order_id))
            if cancel_order_obj and cancel_order_obj.lines:
                for each_line in cancel_order_obj.lines:
                    if each_line.state != False:
                        print('each_line.state',each_line.state)
                        each_line.write({'state':'cancel'})
                        
                        
                
                if cancel_order_obj.state == 'draft':
                    cancel_order = cancel_order_obj.unlink()
                    return cancel_order

                if cancel_order_obj.state == 'paid':
                    cancel_order_obj.refund()
    ...
    def f_send_to_kitchen(self, orders):
        send_to_kitchen = False
        if orders:
            
            for order in orders:
                print(order['data']['lines'])
                for line in  order['data']['lines']:
                    print(line[2])
                    if line[2].get('ms_info'):
                        pos_id = line[2].get('ms_info').get('created').get('pos').get('id')
                        print(line[2].get('ms_info'))
                        print(pos_id)
                        if pos_id:  
                            pos_ids = self.env['pos.config'].search([('id', '=', pos_id)])
                            send_to_kitchen = pos_ids['send_to_kitchen']
                            
                        print('send_to_kitchen',send_to_kitchen)
    ...
    def create_from_ui(self, orders):
        ...
        #added by Falak/Ahmad 20190718
        send_to_kitchen = self.f_send_to_kitchen(orders)
    ...
    def _process_order(self, order):
        ...
        draft_order_ids = self.search([('pos_reference', '=', submitted_references)])
        print(draft_order_ids)

        for draft_order in draft_order_ids:
            print('1',draft_order)
            print('1',draft_order.id)
    ...
    def broadcast_order_data(self, new_order):
        ...
        xx_cookers_id = self.env['res.users'].search([('kitchen_screen_user', '=', 'cook')], limit=1)
...
#                                         print('vals[orders]',vals['orders'][0]['order_id'])
                            ...
                            notifications.append(
                                    ((self._cr.dbname, 'pos.order.line', 999 ), {'screen_display_data': vals})) # cook_user_id.id / updated by Falak/Ahmad 20190718
            ...
            #                     ((self._cr.dbname, 'pos.order.line', 999 ), {'screen_display_data': vals})) # manager_id.id / updated by Falak/Ahmad 20190718
                    ...
                    if user and user.kitchen_screen_user != 'cook':
                        notifications.append(
                        ((self._cr.dbname, 'pos.order.line', 999), {'screen_display_data': vals}))#/ updated by Falak
...
class PosOrderLines(models.Model):
    ...
    def update_orderline_state(self, vals):
        ...
        notifications = []
        notifications.append(
            ((self._cr.dbname, 'pos.order.line', 999 ), {'order_line_state': vals})) # order_line.create_uid.id updated by falak/ahmad 20190718
        self.env['bus.bus'].sendmany(notifications)
        return res
        ...
        if vals:
            for val in vals:
                ...
                if state:
                    ...
                    val['state'] = state
                    notifications.append(
                        [(self._cr.dbname, 'pos.order.line', 999 ), {'order_line_state': val}]) # order_line.create_uid.id updated by falak/ahmad 20190718
    ...
    state = fields.Selection(
        selection=[("waiting", "Waiting"), ("preparing", "Preparing"), ("delivering", "Waiting/deliver"),
                   ("done", "Done"), ("cancel", "Cancelled")], default="waiting")
